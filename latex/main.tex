...
% Định nghĩa lại style cho khối Code SQL (Đã Fix lỗi Tiếng Việt)
% Cần thêm thư viện breakable để cho phép ngắt trang
\tcbuselibrary{listingsutf8, skins, breakable} 

\newtcblisting{sqlcode}[2][]{
    breakable, % QUAN TRỌNG: Cho phép code tự xuống trang mới
    enhanced,
    arc=0mm,
    colback=gray!5,
    ...
    listing options={
        ...
        breaklines=true, % Tự động xuống dòng khi câu lệnh quá dài
...
% Định nghĩa lệnh để format comment Tiếng Việt cho đẹp (Màu xanh + Nghiêng)
\newcommand{\vncomment}[1]{\textit{\color{green!50!black}#1}}
...
5 & Records & \entity{LOAN} & \entity{BOOK\_COPY} & N : 1 & Một bản sao có nhiều lịch sử giao dịch mượn \\ \hline
\end{tabular}
\end{table}
...
\textbf{5. Records (N:1) - LOAN đến BOOK\_COPY}
\begin{itemize}
    \item Một bản sao sách có thể có nhiều lịch sử giao dịch mượn khác nhau (mượn, trả, rồi lại mượn)
    \item Mỗi giao dịch mượn ghi nhận một bản sao sách cụ thể
    \item Ràng buộc: \attr{CopyID} trong \entity{LOAN} là khóa ngoại tham chiếu đến \entity{BOOK\_COPY}
    \item Một bản sao tại một thời điểm chỉ có tối đa một giao dịch đang hoạt động
\end{itemize}
...
\begin{itemize}
    ...
    \item Records (N:1) - \entity{LOAN} → \entity{BOOK\_COPY}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Ví dụ:} Một nhân viên chỉ có một văn phòng làm việc (\entity{EMPLOYEE} $\leftrightarrow$ \entity{OFFICE})
    \item \textbf{Lưu ý:} Trong hệ thống thư viện này, tất cả quan hệ đều là 1:N hoặc M:N, không có quan hệ 1:1
\end{itemize}
...
\subsubsection{Xác nhận lại Mô hình Quan hệ từ Báo cáo 2}

Sau khi hoàn thành phân tích chuản hóa 1NF, 2NF, 3NF, chúng tôi xác nhận lại rằng \textbf{Mô hình Quan hệ (Relational Data Model)} đã thiết kế trong Báo cáo 2 là hoàn toàn chính xác:

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,keepaspectratio]{images/RDM_Library_Management.pdf}
\caption{Sơ đồ Mô hình Quan hệ (RDM) từ Báo cáo 2 - Đã xác nhận đạt 1NF, 2NF, 3NF}
\label{fig:rdm_report3_final}
\end{figure}

\textbf{Kết quả xác nhận:}
\begin{itemize}
    \item \textbf{Tất cả 7 bảng đều đạt 1NF:} Thuộc tính nguyên thủy, không đa trị, không nhóm lặp lại
    \item \textbf{Tất cả 7 bảng đều đạt 2NF:} Không có phụ thuộc hàm một phần
    \item \textbf{Tất cả 7 bảng đều đạt 3NF:} Không có phụ thuộc hàm truyền tải
    \item \textbf{Không có bất thường (anomalies):} Insert, Update, Delete đều an toàn
\end{itemize}

\textbf{Kết luận:} Mô hình Quan hệ từ Báo cáo 2 đã được thiết kế đúng đắn từ đầu, không cần điều chỉnh sau chuẩn hóa.
...
\begin{sqlcode}{Listing 1: Tạo cơ sở dữ liệu LibraryDB}
-- (* \vncomment{Tạo cơ sở dữ liệu} *)
CREATE DATABASE LibraryDB;
GO

-- (* \vncomment{Sử dụng cơ sở dữ liệu vừa tạo} *)
USE LibraryDB;
\end{sqlcode}

\subsubsection{Tổng quan cơ sở dữ liệu}

\textbf{Tổng quan các bảng:}
\begin{itemize}
    \item \textbf{PUBLISHER} (1) -- $\leftrightarrow$ -- \textbf{BOOK} (N): Một NXB xuất bản nhiều sách
    \item \textbf{AUTHOR} (M) -- $\leftrightarrow$ -- \textbf{BOOK} (N): Quan hệ nhiều-nhiều qua BOOK\_AUTHOR
    \item \textbf{BOOK} (1) -- $\leftrightarrow$ -- \textbf{BOOK\_COPY} (N): Một sách có nhiều bản sao vật lý
    \item \textbf{MEMBER} (1) -- $\leftrightarrow$ -- \textbf{LOAN} (N): Một thành viên có nhiều giao dịch mượn
    \item \textbf{BOOK\_COPY} (1) -- $\leftrightarrow$ -- \textbf{LOAN} (N): Một bản sao có nhiều lịch sử giao dịch
\end{itemize}

\textbf{Xác nhận thiết kế:}
\begin{itemize}
    \item ✅ Tất cả 7 bảng đều có Primary Key rõ ràng
    \item ✅ 7 Foreign Keys đảm bảo tính toàn vẹn tham chiếu
    \item ✅ Quan hệ N:1 giữa LOAN và BOOK\_COPY (đã sửa đúng theo yêu cầu)
    \item ✅ Bảng trung gian BOOK\_AUTHOR cho quan hệ M:N
\end{itemize}

\subsubsection{Xóa bảng nếu tồn tại (để tránh lỗi)}

\begin{sqlcode}{Xóa bảng theo thứ tự đúng}
-- (* \vncomment{Xóa bảng theo thứ tự (con trước, cha sau)} *)
...
\begin{sqlcode}{Tạo bảng PUBLISHER}
...
CREATE TABLE BOOK (
    ...
    -- (* \vncomment{Primary Key} *)
    CONSTRAINT PK_BOOK PRIMARY KEY (BookID),

    -- (* \vncomment{Foreign Key} *)
    CONSTRAINT FK_BOOK_PUBLISHER FOREIGN KEY (PublisherID)
        REFERENCES PUBLISHER(PublisherID)
        ON UPDATE CASCADE
        ON DELETE NO ACTION,

    -- (* \vncomment{Unique Constraint} *)
    CONSTRAINT UQ_BOOK_ISBN UNIQUE (ISBN),

    -- (* \vncomment{Check Constraints} *)
    ...
    -- (* \vncomment{Ngắt dòng chuỗi Regex để tránh tràn trang báo cáo} *)
    CONSTRAINT CK_BOOK_ISBN
        CHECK (ISBN IS NULL
               -- (* \vncomment{Định dạng ISBN-10: X-XXXXX-XXXX} *)
               OR ISBN LIKE '[0-9][0-9][0-9]-' + 
                            '[0-9][0-9][0-9][0-9][0-9]%'
               -- (* \vncomment{Định dạng ISBN-13: XXX-XXXXX-XXXX-X} *)
               OR ISBN LIKE '[0-9][0-9][0-9]-' + 
                            '[0-9][0-9][0-9][0-9][0-9]-' +
                            '[0-9][0-9][0-9][0-9][0-9]-' +
                            '[0-9]')
...
\begin{sqlcode}{Tạo bảng BOOK\_COPY}
...
-- Tạo Filtered Unique Index: Mỗi bản sao chỉ được mượn bởi 1 người khi chưa trả
...
\textbf{Giải thích ràng buộc:}
\begin{itemize}
    \item \texttt{UQ\_LOAN\_CopyID\_Active} (Filtered Unique Index): Mỗi bản sao chỉ được mượn bởi một người tại một thời điểm (chỉ áp dụng khi \texttt{ReturnDate IS NULL})
...
\begin{sqlcode}{Tạo bảng BOOK\_AUTHOR}
...
\subsubsection{Sơ đồ tổng quan cơ sở dữ liệu}

Dưới đây là sơ đồ tổng quan tất cả các bảng và mối quan hệ trong cơ sở dữ liệu LibraryDB sau khi đã tạo xong tất cả các bảng:

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,keepaspectratio]{images/dbdiagram.pdf}
\caption{Sơ đồ tổng quan cơ sở dữ liệu LibraryDB - Tất cả 7 bảng và mối quan hệ}
\label{fig:dbdiagram}
\end{figure}
...
\begin{sqlcode}{Stored Procedure mượn sách}
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- (* \vncomment{1. Kiểm tra thành viên tồn tại} *)
        ...
        -- (* \vncomment{2. Kiểm tra bản sao sách tồn tại và AVAILABLE} *)
        IF NOT EXISTS (
...
\begin{sqlcode}{Stored Procedure trả sách}
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- (* \vncomment{1. Lấy thông tin giao dịch mượn} *)
        ...
        -- (* \vncomment{2. Tính phí phạt (nếu quá hạn)} *)
        SET @DaysOverdue = DATEDIFF(DAY, @DueDate, @ReturnDate);

        IF @DaysOverdue > 0
            SET @Fine = @DaysOverdue * 5000; -- (* \vncomment{5000 VNĐ/ngày} *)
        ELSE
            SET @Fine = 0;

        -- (* \vncomment{3. Cập nhật giao dịch mượn} *)
        UPDATE LOAN
        SET ReturnDate = @ReturnDate,
            OverdueFine = @Fine
        WHERE LoanID = @LoanID;

        -- (* \vncomment{4. Cập nhật trạng thái bản sao} *)
        UPDATE BOOK_COPY
...
\begin{sqlcode}{Table-Valued Function}
...
\begin{sqlcode}{Trigger kiểm tra thành viên bị quá hạn}
...
BEGIN
    ...
    -- (* \vncomment{Kiểm tra thành viên có sách quá hạn chưa trả} *)
    IF EXISTS (
        SELECT 1 FROM LOAN
    ...
    -- (* \vncomment{Nếu không vi phạm, thực hiện INSERT} *)
...
\begin{sqlcode}{Trigger tự động tính phí phạt}
...
BEGIN
    ...
    -- (* \vncomment{Chỉ xử lý khi ReturnDate được cập nhật từ NULL sang có giá trị} *)
...
\begin{sqlcode}{Liệt kê sách đang được mượn}
SELECT
    l.LoanID,
...
\begin{sqlcode}{Thống kê sách được mượn nhiều nhất}
SELECT TOP 10
    b.BookID,
    b.Title,
    b.ISBN,
...
\begin{sqlcode}{Thành viên có sách quá hạn}
SELECT
    m.MemberID,
    m.Name,
    m.Phone,
...
\begin{sqlcode}{INSERT dữ liệu vào PUBLISHER}
...
\begin{sqlcode}{INSERT dữ liệu vào AUTHOR}
...
\begin{sqlcode}{INSERT dữ liệu vào MEMBER}
...
\begin{sqlcode}{INSERT dữ liệu vào BOOK}
...
\begin{sqlcode}{INSERT dữ liệu vào BOOK_AUTHOR}
...
\begin{sqlcode}{INSERT dữ liệu vào BOOK\_COPY}
...
\begin{sqlcode}{INSERT dữ liệu vào LOAN}
...
\begin{sqlcode}{Mượn sách thành công}
...
\begin{sqlcode}{Trả sách đúng hạn}
...
\begin{sqlcode}{Trả sách quá hạn}
EXEC dbo.usp_ReturnBook
    @CopyID = 5,
...
\begin{sqlcode}{Mượn sách khi sách không có sẵn}
EXEC dbo.usp_BorrowBook
    @MemberID = 2,
...
\begin{sqlcode}{Test ufn\_GetBooksByAuthor}
...
\begin{sqlcode}{Test ufn_CalculateOverdueFine}
-- (*Trả đúng hạn*)
...
\begin{sqlcode}{Sau khi trả sách quá hạn}
-- Trả sách quá hạn
...
\begin{sqlcode}{Thống kê sách phổ biến}
SELECT TOP 10
    b.BookID,
    b.Title,
...
\begin{sqlcode}{Danh sách thành viên quá hạn}
SELECT
    m.MemberID,
    m.Name,
    m.Phone,
...
\begin{sqlcode}{Thống kê theo nhà xuất bản}
SELECT
    p.PublisherID,
...
\begin{sqlcode}{Thống kê doanh thu phí phạt}
SELECT
