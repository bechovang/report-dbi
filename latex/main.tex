...
\begin{titlepage}
    ...
    %\includegraphics[width=3cm]{fpt_logo.png}
    \vspace{0.5cm}

    {\Huge\bfseries 123rrr ĐẠI HỌC FPT}\\
    {\Huge\bfseries FPT UNIVERSITY}\\[1cm]

    {\Large\textbf{Thể loại:}} Assignment\\[0.3cm]
    {\Large\textbf{CHỦ ĐỀ:}}\\[0.3cm]
    {\huge\bfseries XÂY DỰNG HỆ THỐNG CƠ SỞ DỮ LIỆU\\ QUẢN LÝ THƯ VIỆN}\\[0.5cm]
    {\Large BUILDING A DATABASE SYSTEM FOR\\ LIBRARY MANAGEMENT}\\[2cm]

    {\Large\textbf{MÔN HỌC:} HỆ THỐNG CƠ SỞ DỮ LIỆU}\\[0.3cm]
    {\Large\textbf{COURSE:} DATABASE SYSTEMS (DBI202)}\\[0.3cm]
    {\Large\textbf{LỚP:} SE2043}\\[2cm]

    \begin{flushleft}
        {\Large\textbf{GIẢNG VIÊN HƯỚNG DẪN:}}\\
        {\Large VŨ THANH PHONG}\\[1cm]

        {\Large\textbf{NHÓM 2}}\\[0.5cm]

        \begin{tabular}{@{}ll@{}}
        1.  & Nguyễn Ngọc Phúc \quad \quad MSSV: SE203055 \\
        2.  & Thân Nhật Huy \quad MSSV: SE203317 \\
        3.  & Võ Hoàng Đình Trường \quad \quad MSSV: SE203475 \\
        4.  & xxxx \quad \quad MSSV: SE171267 \\
        5.  & xxxxx \quad \quad \quad MSSV: SS170814 \\
        6.  & xxxxxx \quad MSSV: SE201706 \\
        7.  & xxxxxxxx \quad \quad MSSV: SE191138 \\
        \end{tabular}
    \end{flushleft}
...
\hline
\textbf{MSSV} & \textbf{Họ và tên} & \textbf{Phân công nhiệm vụ} & \textbf{Đánh giá} \\ \hline
SE200064 & Lê Khôi Nguyên & Thiết kế E/R Diagram, kiểm tra chất lượng tài liệu & Điểm: 10 \\ \hline
SE191001 & Nguyễn Thị Thanh Thảo & Thu thập và phân tích yêu cầu người dùng & Điểm: 10 \\ \hline
SE201706 & Trần Đình Bảo Hoàng & Xác định mối quan hệ giữa các thực thể & Điểm: 2 \\ \hline
SE201711 & Đoàn Đức Nguyên & Xác định mối quan hệ giữa các thực thể & Điểm: 2 \\ \hline
SE191138 & Nguyễn Tiến Dũng & Xác định các thực thể và thuộc tính & Điểm: 7 \\ \hline
SE171267 & Trần Phương Uyên & Tổng hợp các tài liệu và viết báo cáo 1 & Điểm: 9 \\ \hline
SS170814 & Lê Kim Ngân & Phân công nhiệm vụ, kiểm tra tài liệu & Điểm: 10 \\ \hline
\end{tabular}
\end{table}
...
\begin{tikzpicture}[
    ...
    entity/.style={rectangle, draw=black, fill=blue!10, very thick, minimum size=1cm, minimum width=2.5cm, align=center},
    weakentity/.style={rectangle, draw=black, double, fill=yellow!10, very thick, minimum size=1cm, minimum width=2.5cm, align=center},
    relationship/.style={diamond, draw=black, fill=green!10, very thick, minimum size=1cm, aspect=2, align=center},
    attribute/.style={ellipse, draw=black, fill=white, thin, minimum width=1.5cm, align=center},
    pk/.style={attribute, draw=black, fill=yellow!30, very thick}
]

% Define entities
\node[entity] (publisher) {\textbf{PUBLISHER}\\ \small PublisherID (PK)\\ \small Name\\ \small Address\\ \small ContactNumber};

\node[entity, below left=of publisher] (book) {\textbf{BOOK}\\ \small BookID (PK)\\ \small ISBN\\ \small Title\\ \small SubjectCategory\\ \small PublicationYear\\ \small PublisherID (FK)};

\node[entity, above left=of book] (author) {\textbf{AUTHOR}\\ \small AuthorID (PK)\\ \small Name\\ \small Nationality\\ \small YearOfBirth};

\node[entity, below right=of book] (bookcopy) {\textbf{BOOK\_COPY}\\ \small CopyID (PK)\\ \small BookID (FK)\\ \small Condition\\ \small Status};

\node[entity, below=of bookcopy] (loan) {\textbf{LOAN}\\ \small LoanID (PK)\\ \small MemberID (FK)\\ \small CopyID (FK)\\ \small BorrowDate\\ \small DueDate\\ \small ReturnDate\\ \small OverdueFine};

\node[entity, right=of loan] (member) {\textbf{MEMBER}\\ \small MemberID (PK)\\ \small Name\\ \small Address\\ \small Phone\\ \small MembershipType};

% Junction table for M:N relationship
\node[entity, dashed, fill=orange!10, above=of book] (bookauthor) {\textbf{BOOK\_AUTHOR}\\ \small (Junction Table)\\ \small BookID (FK, PK)\\ \small AuthorID (FK, PK)};

% Define relationships
\node[relationship, below right=0.8cm and 0.2cm of publisher] (publishes) {Publishes\\ 1:N};

\node[relationship, above right=0.5cm and 0.5cm of author] (writes) {Writes\\ M:N};

\node[relationship, below right=0.8cm and 0.2cm of book] (has) {Has\\ 1:N};

\node[relationship, left=0.5cm of loan] (processes) {Processes\\ 1:N};

\node[relationship, above=0.5cm of loan] (records) {Records\\ 1:1};

% Draw relationship lines
% Publishes: PUBLISHER -> BOOK
\draw[->, very thick] (publisher) -- (publishes);
\draw[->, very thick] (publishes) -- (book);

% Writes: AUTHOR <-> BOOK (via BOOK_AUTHOR)
\draw[->, very thick] (author) -- (writes);
\draw[->, very thick] (writes) -- (bookauthor);
\draw[->, very thick, dashed] (bookauthor) -- (book);
\draw[->, very thick, dashed] (book) -- (bookauthor);

% Has: BOOK -> BOOK_COPY
\draw[->, very thick] (book) -- (has);
\draw[->, very thick] (has) -- (bookcopy);

% Processes: MEMBER -> LOAN
\draw[->, very thick] (member) -- (processes);
\draw[->, very thick] (processes) -- (loan);

% Records: LOAN -> BOOK_COPY
\draw[->, very thick] (loan) -- (records);
\draw[->, very thick] (records) -- (bookcopy);

% Add cardinality labels
\node[font=\footnotesize, fill=white] at ($(publisher)!0.5!(publishes)$) {1};
\node[font=\footnotesize, fill=white] at ($(publishes)!0.5!(book)$) {N};

\node[font=\footnotesize, fill=white] at ($(author)!0.5!(writes)$) {M};
\node[font=\footnotesize, fill=white] at ($(writes)!0.5!(bookauthor)$) {};

\node[font=\footnotesize, fill=white] at ($(book)!0.5!(has)$) {1};
\node[font=\footnotesize, fill=white] at ($(has)!0.5!(bookcopy)$) {N};

\node[font=\footnotesize, fill=white] at ($(member)!0.5!(processes)$) {1};
\node[font=\footnotesize, fill=white] at ($(processes)!0.5!(loan)$) {N};

\node[font=\footnotesize, fill=white] at ($(loan)!0.5!(records)$) {1};
\node[font=\footnotesize, fill=white] at ($(records)!0.5!(bookcopy)$) {1};

% Legend
\node[draw, fill=white, anchor=south east] at (current bounding box.north east) {
    \begin{tabular}{@{}cl@{}}
    \tikz\node[entity, minimum size=0.5cm] {}; & Entity \\
    \tikz\node[relationship, minimum size=0.5cm] {}; & Relationship \\
    \tikz\node[entity, dashed, minimum size=0.5cm] {}; & Junction Table \\
    \end{tabular}
};
