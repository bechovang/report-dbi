...
\begin{tblr}{
  colspec = {|l|l|l|l|l|},
  vlines, hlines,
  rows = {m}, % Căn giữa tất cả các dòng theo chiều dọc (Middle)
  columns = {l}, % Căn trái nội dung trong cột
  cell{2}{1} = {r=6}{c}, % Gộp 6 dòng ở cột 1 và căn giữa (Center)
  cell{2}{2} = {r=6}{c}, % Gộp 6 dòng ở cột 2 và căn giữa (Center)
  cell{7}{3} = {c},      % Căn giữa PublisherID ở cột 3
}
\textbf{Tên bảng} & \textbf{Khóa chính} & \textbf{Khóa ngoại} & \textbf{Thuộc tính} & \textbf{Kiểu dữ liệu} \\
BOOK & \underline{BookID} & & BookID & INT \\
     & & & ISBN & VARCHAR(17) \\
     & & & Title & VARCHAR(255) \\
...
\begin{tabular}{@{}|l|l|l|l|@{}}
\hline
\textbf{Tên bảng} & \textbf{Khóa chính} & \textbf{Khóa ngoại} & \textbf{Thuộc tính} & \textbf{Kiểu dữ liệu} \\ \hline
\multirow{4}{*}{\texttt{BOOK\_COPY}} & \multirow{4}{*}{\underline{\texttt{CopyID}}} & \multirow{2}{*}{\texttt{BookID}} & \texttt{CopyID} & INT \\ \cline{4-5}
& & & \texttt{BookID} & INT \\ \cline{4-5}
& & & \texttt{Condition} & VARCHAR(50) \\ \cline{4-5}
& & & \texttt{Status} & VARCHAR(20) \\ \hline
\end{tabular}
\end{table}
...
\hline
\textbf{Tên bảng} & \textbf{Khóa chính} & \textbf{Khóa ngoại} & \textbf{Thuộc tính} & \textbf{Kiểu dữ liệu} \\ \hline
\multirow{7}{*}{\texttt{LOAN}} & \multirow{7}{*}{\underline{\texttt{LoanID}}} & \multirow{2}{*}{\texttt{MemberID}, \texttt{CopyID}} & \texttt{LoanID} & INT \\ \cline{4-5}
& & & \texttt{MemberID} & INT \\ \cline{4-5}
& & & \texttt{CopyID} & INT \\ \cline{4-5}
& & & \texttt{BorrowDate} & DATE \\ \cline{4-5}
& & & \texttt{DueDate} & DATE \\ \cline{4-5}
& & & \texttt{ReturnDate} & DATE \\ \cline{4-5}
& & & \texttt{OverdueFine} & DECIMAL(10,2) \\ \hline
\end{tabular}
\end{table}
...
\begin{tabular}{@{}|l|l|l|l|@{}}
\hline
\textbf{Tên bảng} & \textbf{Khóa chính} & \textbf{Khóa ngoại} & \textbf{Mô tả} \\ \hline
\multirow{2}{*}{\texttt{BOOK\_AUTHOR}} & \multirow{2}{*}{\underline{\texttt{BookID}}, \underline{\texttt{AuthorID}}} & \texttt{BookID} $\rightarrow$ \texttt{BOOK(BookID)} & Khóa ngoại đến BOOK \\ \cline{3-4}
& & \texttt{AuthorID} $\rightarrow$ \texttt{AUTHOR(AuthorID)} & Khóa ngoại đến AUTHOR \\ \hline
\end{tabular}
\end{table}
...
\texttt{MEMBER}(\underline{\texttt{MemberID}}, \texttt{Name}, \texttt{Address}, \texttt{Phone}, \texttt{MembershipType}) \\
\texttt{BOOK}(\underline{\texttt{BookID}}, \texttt{ISBN}, \texttt{Title}, \texttt{SubjectCategory}, \texttt{PublicationYear}, \texttt{PublisherID}^*) \\
\texttt{BOOK\_COPY}(\underline{\texttt{CopyID}}, \texttt{BookID}^\dagger, \texttt{Condition}, \texttt{Status}) \\
\texttt{LOAN}(\underline{\texttt{LoanID}}, \texttt{MemberID}^\ddagger, \texttt{CopyID}^\ddagger, \texttt{BorrowDate}, \texttt{DueDate}, \texttt{ReturnDate}, \texttt{OverdueFine}) \\
\texttt{BOOK\_AUTHOR}(\underline{\texttt{BookID}^\dagger}, \underline{\texttt{AuthorID}^\ddagger})
\end{aligned}
$$

\textbf{Chú thích:}
\begin{itemize}
    \item $^*$ \texttt{PublisherID} là khóa ngoại tham chiếu \texttt{PUBLISHER(PublisherID)}
    \item $^\dagger$ \texttt{BookID} là khóa ngoại tham chiếu \texttt{BOOK(BookID)}
    \item $^\ddagger$ \texttt{MemberID} là khóa ngoại tham chiếu \texttt{MEMBER(MemberID)}
    \item $^\ddagger$ \texttt{CopyID} là khóa ngoại tham chiếu \texttt{BOOK\_COPY(CopyID)}
    \item Dấu gạch dưới ($\underline{\text{...}}$) chỉ Khóa chính (Primary Key)
\end{itemize}
...
\begin{tabular}{@{}|l|l|l|@{}}
\hline
\textbf{STT} & \textbf{Tên bảng} & \textbf{Khóa chính (PK)} & \textbf{Khóa ngoại (FK)} \\ \hline
1 & \texttt{PUBLISHER} & \texttt{PublisherID} & - \\ \hline
2 & \texttt{AUTHOR} & \texttt{AuthorID} & - \\ \hline
3 & \texttt{MEMBER} & \texttt{MemberID} & - \\ \hline
4 & \texttt{BOOK} & \texttt{BookID} & \texttt{PublisherID} $\rightarrow$ \texttt{PUBLISHER} \\ \hline
5 & \texttt{BOOK\_COPY} & \texttt{CopyID} & \texttt{BookID} $\rightarrow$ \texttt{BOOK} \\ \hline
6 & \texttt{LOAN} & \texttt{LoanID} & \texttt{MemberID} $\rightarrow$ \texttt{MEMBER} \\
& & & \texttt{CopyID} $\rightarrow$ \texttt{BOOK\_COPY} \\ \hline
7 & \texttt{BOOK\_AUTHOR} & \texttt{BookID, AuthorID} & \texttt{BookID} $\rightarrow$ \texttt{BOOK} \\
& & & \texttt{AuthorID} $\rightarrow$ \texttt{AUTHOR} \\ \hline
\end{tabular}
\end{table}
...
\item \textbf{Bộ ký tự cuối đường:} 1 = một, N = nhiều, M,N = nhiều-nhiều
\end{itemize}
