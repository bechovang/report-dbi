...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/erd_diagram.pdf}
\caption{Sơ đồ Thực thể - Mối quan hệ (ERD) cho hệ thống Quản lý Thư viện}
\label{fig:erd}
\end{figure}

\textbf{Giải thích các mối quan hệ chính:}
\begin{enumerate}
\item \textbf{PUBLISHER (1) --- (N) BOOK:} Một nhà xuất bản xuất bản nhiều sách
\item \textbf{AUTHOR (M) --- (N) BOOK:} Mối quan hệ nhiều-nhiều thông qua bảng trung gian BOOK\_AUTHOR
\item \textbf{BOOK (1) --- (N) BOOK\_COPY:} Một đầu sách có nhiều bản sao vật lý
\item \textbf{MEMBER (1) --- (N) LOAN:} Một thành viên có thể có nhiều giao dịch mượn
\item \textbf{BOOK\_COPY (1) --- (N) LOAN:} Một bản sao có thể được mượn nhiều lần theo thời gian
\end{enumerate}
